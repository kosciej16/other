...
create or replace view tmp as select m.*, r.score, re.* from Movies m join Ratings r on m.mID = r.mID join Reviewers re on r.rID = re.rID;

select * from Movies m
left join Ratings ra
    inner join Reviewers re on re.rID = ra.riD
    on ra.mID = m.mID and re.name = 'Daniel Lewis'
    where ra.rID is null;
